...
function resolveTitle(article: Article | null, fallback: string): string {
  return cleanText(article?.title || article?.Title || '') || fallback;
}

function resolveBlocks(article: Article | null): BlockNode[] {
  const content = article?.content ?? article?.Content;
  return Array.isArray(content) ? content : [];
}

function resolveCoverUrl(article: Article | null): string {
  return cleanText(
...
export default function ArticleScreen() {
  ...
  const blocks = useMemo(() => resolveBlocks(article), [article]);
  const coverUrl = useMemo(() => resolveCoverUrl(article), [article]);
  const screenTitle = useMemo(() => resolveTitle(article, titleFallback), [article, titleFallback]);

  if (!articleSlug) {
    return (
      <ThemedView style={styles.centerState}>
        <ThemedText type="subtitle">Missing article slug</ThemedText>
      </ThemedView>
    );
  }
  ...
  if (error) {
    return (
      <ThemedView style={styles.centerState}>
        <ThemedText type="subtitle">Could not load article</ThemedText>
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <ThemedText style={styles.meta}>/{articleSlug}</ThemedText>
        ...
        {blocks.length ? (
          ...
          <ThemedText style={styles.paragraph}>No structured article content available yet.</ThemedText>
        )}
      </ScrollView>
    </ThemedView>
  );
